...
The Linux native toolchain is not part of Git/GitDependencies. Alpha.17 installs its authoritative payload in the shared UECI cache (`toolchains/installed/linux-x64/<version>`) and exposes it to UBT through the normal `Engine/Extras/ThirdPartyNotUE/SDKs/...` projection. On a cold code-plugin build, toolchain acquisition runs concurrently with UBT preparation and synthetic-host generation. Linux prefers native `tar` extraction (and `pigz` when already present) with a managed fallback.
