...
Generated managed outputs are also cached by Epic commit. UECI restores UBT/shared-project outputs and `Engine/Intermediate/Build/BuildRules` into the COW upper before mounting, reuses a complete existing UBT output when possible, and invalidates those artifacts automatically when the commit changes. A dynamic profile retry keeps the commit-identical UBT binary but drops cached Engine Rules first, ensuring the expanded source set rebuilds `UE5Rules.dll`.
