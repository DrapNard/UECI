...
Discovery is monotonic and bounded. Linux compiler/SDK payloads are external providers because Epic normally installs them through `Setup.sh` rather than `Commit.gitdeps.xml`. Modern releases use Epic's versioned clang/sysroot projection; UE4.20–4.27 materialized builds may acquire that projection before the first native UBT pass because these generations can fail platform registration before emitting a parseable SDK diagnostic. Pre-4.20 UE4 instead resolves the historical native clang family and never treats an arbitrary runner clang as compatible. The authoritative extracted payload lives outside the Git-managed `Engine/` subtree, and projections are recreated after sparse updates. Legacy Mono UBT retries only the small set of known Linux SDK environment layouts and only while the process reports `No BuildPlatform found for Linux`; unrelated compile/link failures are never retried as SDK guesses. UECI never treats a failed build as permission to materialize all of `Engine/Source`.
